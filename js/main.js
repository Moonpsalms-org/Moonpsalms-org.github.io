...
function initBannerSlider() {
  const slider = document.querySelector('.adaptive-banner-slider');
  if (!slider) return; // 页面没有轮播图时直接返回
  ...
  function updateIndicators() {
    const indicators = indicatorContainer.querySelectorAll('.slider-indicator');
...
// PJAX页面切换后重新初始化
window.addEventListener('pjax:complete', initBannerSlider);
